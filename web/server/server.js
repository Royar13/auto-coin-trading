...
const ERC20JsonPath = path.resolve(process.cwd(), "node_modules/@openzeppelin/contracts/build/ERC20.json");
...
const autoCoinTraderAddr = "0x5CC29cA7465c2925A74055bf7a2f65a000DFFD43";
...
arbitrage.getExchangePath().then(async cycles => {
    ...
    for (let i = 0; i < cycle.length - 2; i++) {
        ...
        if (i === 0) {
            const tokenContract = new web3.eth.Contract(ERC20abi, coinA.address);
            ...
            let transferMethod = tokenContract.methods.transfer(autoCoinTrader.address, amountIn);
